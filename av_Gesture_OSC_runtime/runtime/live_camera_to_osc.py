...
def make_preview_frame(frame_bgr, result=None, prev_cfg=None):
    prev_cfg = prev_cfg or {}
    preview_frame = frame_bgr.copy()
    if result is not None and prev_cfg.get("draw_landmarks", False):
        preview_frame = draw_mediapipe_landmarks(preview_frame, result)
    if prev_cfg.get("mirror", True):
        ...
        while True:
            ...
            if num_hands == 0:
                ...
                if prev_cfg.get("show_window", True):
                    preview_frame = make_preview_frame(frame_bgr, prev_cfg=prev_cfg)
            ...
            if prev_cfg.get("show_window", True):
                preview_frame = make_preview_frame(frame_bgr, result, prev_cfg)
                preview_frame = draw_cluster_overlay(
                    preview_frame,
                    cluster=display_cluster,
                    confidence=display_confidence,
