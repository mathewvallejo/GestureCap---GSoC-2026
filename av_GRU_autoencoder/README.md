...
```text
av_autoencoder/av_GRU_autoencoder
```
...
../Window_Data      shared Stage 2 window output folder
../Model_Outputs    shared Stage 2 model output folder
../Video_Data       shared source video/audio folder
...
```text
Window_Data/
├── video_file_here_win000000.npz
├── video_file_here_win000001.npz
├── window_manifest.csv
└── window_summary.json
```
...
The builder first tries the `video` path recorded in each Stage 1 CSV, then falls back to matching filenames in `--audio-source-dir`, for example `video_file_here_landmarks.csv` -> `video_file_here.mp4`.
