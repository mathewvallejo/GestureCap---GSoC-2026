...
Command paths below assume you run them from this folder:

```text
av_autoencoder/av_Camera_Calibration_Preprocess
```

Relative path reminders:

```text
..                  the av_autoencoder project root
../Video_Data       shared source video folder
../Feature_Data     shared Stage 1 landmark output folder
../Calibration_Data shared calibration image folder
camera_id_here      replace with the camera calibration label
video_file_here     replace with the source video stem or filename
```

## Setup

From the `av_autoencoder` project root:

```bash
cd av_Camera_Calibration_Preprocess
...
python scripts/capture_chessboard_images.py \
  --camera 0 \
  --camera-id camera_id_here \
  --output-dir "../Calibration_Data/camera_id_here" \
...
python scripts/calibrate_camera.py \
  --images "../Calibration_Data/camera_id_here" \
  --camera-id camera_id_here \
  --config configs/calibration_config.yaml \
  --output "outputs/calibration/camera_id_here_intrinsics.npz"
```
...
python scripts/extract_landmarks_undistorted.py \
  --video "../Video_Data/video_file_here.mp4" \
  --camera-calibration "outputs/calibration/camera_id_here_intrinsics.npz" \
  --output "../Feature_Data/video_file_here_landmarks.csv"
```
...
python scripts/undistort_video.py \
  --input "../Video_Data/video_file_here.mp4" \
  --camera-calibration "outputs/calibration/camera_id_here_intrinsics.npz" \
  --output "../Video_Data/video_file_here_undistorted.mp4"
```
