...
```text
av_autoencoder/av_Gesture_OSC_runtime
```
...
../Model_Outputs    shared Stage 2 model output folder
../Video_Data       shared source/replay video folder
run_name_here       replace with the Stage 2 output run name
video_file_here     replace with a source or replay video filename
...
```text
../Model_Outputs/run_name_here/export_for_runtime/
```
...
```yaml
runtime_model:
  artifact_dir: "../Model_Outputs/run_name_here/export_for_runtime"
```
...
python runtime/live_camera_to_osc.py \
  ...
  --camera 0 \
  --model-dir "../Model_Outputs/run_name_here/export_for_runtime"
```
...
calibration:
  enabled: true
  camera_calibration_path: "../av_Camera_Calibration_Preprocess/outputs/calibration/camera_id_here_intrinsics.npz"
  alpha: 0.0
```
...
Use the artifact folder exported by Stage 2. Replace `run_name_here` with the Stage 2 run you want to test:
