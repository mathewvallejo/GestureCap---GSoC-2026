...
CAMERA_STARTUP_TIMEOUT_SECONDS = 20.0
CAMERA_STARTUP_READY_TEXT = "Camera frame received."


def stop_pose2osc_process(
    process: subprocess.Popen[str],
    log,
    *,
    graceful_timeout: float = GRACEFUL_STOP_TIMEOUT_SECONDS,
    force_timeout: float = FORCE_STOP_TIMEOUT_SECONDS,
) -> int | None:
    ...
    try:
        return process.wait(timeout=force_timeout)
    except subprocess.TimeoutExpired:
        process.kill()
        return process.wait(timeout=force_timeout)
...
class Pose2OSCApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.camera_start_deadline: float | None = None
    ...
    def _build_performance_tab(self) -> None:
        ...
        for column in range(2):
            osc.columnconfigure(column, weight=1, minsize=180)
        ...
        osc_toggles = ttk.Frame(osc)
        osc_toggles.grid(row=1, column=0, columnspan=2, sticky="ew", padx=4, pady=(2, 10))
        for column in range(2):
            osc_toggles.columnconfigure(column, weight=1, minsize=210)
        ttk.Checkbutton(osc_toggles, text="Split axes", variable=self.split_axes_var, command=self._refresh_osc_routes).grid(row=0, column=0, sticky="w", padx=8, pady=2)
        ttk.Checkbutton(osc_toggles, text="Landmark vectors", variable=self.landmark_vectors_var, command=self._refresh_osc_routes).grid(row=0, column=1, sticky="w", padx=8, pady=2)
        ttk.Checkbutton(osc_toggles, text="Unknown predictions", variable=self.send_unknown_var, command=self._refresh_osc_routes).grid(row=1, column=0, sticky="w", padx=8, pady=2)
        ttk.Checkbutton(osc_toggles, text="Show OSC routes", variable=self.show_routes_var, command=self._sync_routes_panel).grid(row=1, column=1, sticky="w", padx=8, pady=2)
    ...
    def _start_process(self, command: list[str], label: str) -> None:
        ...
        self.camera_start_deadline = time.monotonic() + CAMERA_STARTUP_TIMEOUT_SECONDS
    ...
    def _poll_log_queue(self) -> None:
        while True:
            ...
            if CAMERA_STARTUP_READY_TEXT in line:
                self.camera_starting = False
                self.camera_start_deadline = None
    ...
    def _poll_process(self) -> None:
        self._check_camera_startup_timeout()
    ...
    def _check_camera_startup_timeout(self) -> None:
        if (
            self.process is None
            or not self.camera_starting
            or self.camera_start_deadline is None
            or time.monotonic() <= self.camera_start_deadline
        ):
            return

        process = self.process
        self._append_log(
            "ERROR: Camera preview did not start within "
            f"{CAMERA_STARTUP_TIMEOUT_SECONDS:0.0f}s. "
            "Pose2OSC is cleaning up the stuck camera worker. "
            "Close other camera apps or try a different camera index, then start again.\n"
        )
        return_code = stop_pose2osc_process(
            process,
            self._append_log,
