...
def _runtime_log(message: str) -> None:
    if os.environ.get("POSE2OSC_RUNTIME_LOG"):
...
def raw_camera_test(
    *,
    ...
    """Open the camera preview with OpenCV only, without importing MediaPipe."""

    cv2 = _load_cv2_dependency()
    _prepare_preview_window(cv2, True)
    with _open_capture(cv2, camera, width, height) as capture:
        frame_watchdog = _FrameReadWatchdog(camera, announce_first_success=True)
        try:
            while True:
...
def _load_cv2_dependency() -> Any:
    _runtime_log("Camera startup: importing OpenCV...")
    try:
        import cv2  # type: ignore
    except ImportError as exc:
        raise RuntimeError(
            "Camera preview requires OpenCV. Install it with: python -m pip install -e ."
        ) from exc
    _runtime_log("Camera startup: OpenCV imported.")
    return cv2


def _load_mediapipe_dependency() -> Any:
    _runtime_log("Camera startup: importing MediaPipe...")
    try:
        import mediapipe as mp  # type: ignore
    except ImportError as exc:
        raise RuntimeError(
            "Hand tracking requires MediaPipe. Install it with: python -m pip install -e ."
        ) from exc
    _runtime_log("Camera startup: MediaPipe imported.")
    return mp


def _load_camera_dependencies() -> tuple[Any, Any]:
    _runtime_log("Camera startup: loading camera dependencies...")
    return _load_cv2_dependency(), _load_mediapipe_dependency()
...
class _FrameReadWatchdog:
    def __init__(
        ...
        *,
        ...
        announce_first_success: bool = False,
    ) -> None:
        self.camera = camera
        self.timeout_seconds = timeout_seconds
        self.announce_first_success = announce_first_success
        self.announced_success = False
    ...
    def read(self, capture: Any) -> Any | None:
        ...
        if ok and frame is not None:
            self.last_success = time.monotonic()
            if self.announce_first_success and not self.announced_success:
                _runtime_log("Camera frame received.")
                self.announced_success = True
            return frame
...
def _open_capture(
    ...
    _runtime_log(f"Camera startup: opening camera {camera}...")
    backend = getattr(cv2, "CAP_AVFOUNDATION", None)
    if sys.platform == "darwin" and backend is not None:
    ...
    _runtime_log(f"Camera startup: camera {camera} opened.")
...
def _prepare_preview_window(cv2: Any, show: bool) -> None:
    if not show:
        return
    _runtime_log("Camera startup: preparing preview window...")
