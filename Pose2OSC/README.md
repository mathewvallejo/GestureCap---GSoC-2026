...
```bash
.venv/bin/python pose2osc/cli.py camera-test --raw --camera 0
...
Use `camera-test --raw` first to test OpenCV and the webcam without MediaPipe.
Then use `camera-test` to test OpenCV plus MediaPipe hand tracking. If raw mode
works but the normal camera test hangs, the issue is in MediaPipe import or hand
tracking startup rather than camera permissions.
...
camera permissions for Terminal or Python. The Output panel reports the exact
CLI command it launched, camera startup milestones, and a timeout error if the
worker gets stuck before the first camera frame. Pose2OSC cleans up that stuck
worker automatically so the next launch can start fresh.
