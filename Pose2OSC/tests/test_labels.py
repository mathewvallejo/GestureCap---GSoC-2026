...
class LabelTests(unittest.TestCase):
    def test_palette_has_twelve_distinct_colors(self):
        self.assertEqual(len(COLOR_PALETTE), 12)
        self.assertEqual(len(set(COLOR_PALETTE)), 12)
    ...
    def test_generated_labels_sort_in_natural_order(self):
        from pose2osc.manifest import GestureModel

        model = GestureModel()
        for label in ["gesture_1", "gesture_10", "gesture_2"]:
            model.add_samples(label, [[(0.0, 0.0, 0.0)] * 21])

        self.assertEqual(model.labels, ["gesture_1", "gesture_2", "gesture_10"])
