...
class CliConfigTests(unittest.TestCase):
    ...
    def test_raw_camera_test_skips_mediapipe_runtime(self):
        buffer = io.StringIO()
        with (
            contextlib.redirect_stdout(buffer),
            mock.patch("pose2osc.cli.raw_camera_test") as raw_camera_test,
            mock.patch("pose2osc.cli.camera_test") as camera_test,
        ):
            exit_code = main(["camera-test", "--raw", "--camera", "1"])

        self.assertEqual(exit_code, 0)
        raw_camera_test.assert_called_once()
        camera_test.assert_not_called()
        self.assertEqual(raw_camera_test.call_args.kwargs["camera"], 1)
